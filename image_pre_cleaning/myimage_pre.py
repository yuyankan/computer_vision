...
def image_contrast_scale(image,parameters):
    alpha = 1.2
    if 'alpha' not in parameters:
        print('alpha fnot defined, use default 1.2')
    else: 
        alpha = parameters['alpha']
    beta = 0
    if 'beta' not in parameters:
        print('beta not defined, use default 0')
    else:
        beta = parameters['beta']

    image_scale = cv2.convertScaleAbs(image, alpha=alpha, beta=beta)
    return image_scale

def image_contrast_clahe(image, parameters):
    # 3. 增强灰度图的对比度
    ...
    if 'clipLimit' not in parameters:
        print('clipLimit not defined, use default: 2.0')
    else:
        clipLimit = parameters['clipLimit']

 
    if 'tileGridSize' not in parameters:
        print('tileGridSize:  not defined, use default: (8, 8)')
    else:
